...
main() {
    ...
    if [[ "$environment" == "aws-emr-serverless" ]]; then
        ...
        echo "export AWS_ENDPOINT_URL_S3=\"http://localhost:9000\"" > ".env.override"
        echo "export AWS_ACCESS_KEY_ID=admin" >> ".env.override"
        echo "export AWS_SECRET_ACCESS_KEY=password" >> ".env.override"
    fi
}
